...
const int HAND_JUST_BEFORE_TOUCH_SWITCH = 140;  // servo angle for the hand to touch the switch, without activating it
...
void deactivationPretender(int turns = random(4, 12), int positionA = HAND_ON_SWITCH_POSITION - 30, int positionB = HAND_ON_SWITCH_POSITION - 6) {
    boxOpen(NORMAL);
    moveServo(handServo, handServo.read(), positionA, FAST);

    for (int turn = 0; turn <= turns; turn++) {
        moveServo(handServo, positionA, positionB, FAST);
        moveServo(handServo, positionB, positionA, FAST);
    }
...
void loop() {
    ...
    if (switchStatus == HIGH) {  // in case the switch was activated
        ...
        if (isFirstMovement) {  // make sure to always run the basic movement in the first time machine is activated
            ...
            randomScene = random(1, 5);  // generate random number from 1 to 2
//            randomScene = 4;
            if (randomScene == 1) {
                Serial.println("Basic Movement");
